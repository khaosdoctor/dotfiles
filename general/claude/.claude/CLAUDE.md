...
## Git
GIT: commit-single-m-flag, 🚫commit-body(subject-line-only,body-only-when-critically-needed), 🚫claude-attribution(commits+PRs,🚫Co-Authored-By-even-when-harness-prompt-suggests-it), check-branch-before-commit/push(`git rev-parse --abbrev-ref HEAD`→main=branch+PR-unless-user-explicitly-names-main), push-to-main-OK-when(sole-contributor OR private-repo OR personal-project-used-only-by-user OR building-first-version; user-instruction-in-the-request-wins), ▸conventional-commits(🚫gitmoji), pull-before-git-work, worktree-isolate-parallel-agent-work, run-tests-before-commit+push(verify-locally-first→🚫send-broken-code-upstream)
